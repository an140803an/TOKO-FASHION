...
public class CartController {
	...
	@PostMapping("/GioHang")
	public String cart(@RequestParam("id") String product_id, @RequestParam("size") String product_size,
		...
		if (sessionService.get("Order") == null) {
			...
			odd.setSize(product_size);

			listodd.add(odd);
			
			sessionService.set("Order", listodd);
			model.addAttribute("Orders",sessionService.get("Order"));
			...
			for (Orderdetails x : listodd) {
				if (x.getProduct().getProductId().equals(pd.getProductId())
						&& x.getSize().equals(product_size)) {
		...
		System.out.println(x.getOrderDetailsId());
...
//							&& x.getSize().equals(product_color)) {
//						Integer quantity = x.getQuantity() + Integer.valueOf(product_quantity);
//						x.setQuantity(quantity);
//						found = true;
//						break;
...
//		}
	...
	public String Pay(@RequestParam("totalmoney") String totalmoney, Model model) {
		...
		if (User == null) {
			...
			int so = Integer.valueOf(totalmoney);
			if(so != 0) {
				List<Orderdetails> listodd = sessionService.get("Order");
